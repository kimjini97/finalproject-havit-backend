...
public class GroupController {
    ...
    public ResponseDto<GroupDto.Response> createGroup(@ModelAttribute GroupDto.Request groupRequestDto, @AuthenticationPrincipal UserDetailsImpl userDetails) throws IOException {
        return groupService.createGroup(groupRequestDto, userDetails);
    }

    @Operation(summary = "전체 그룹 조회", description = "생성된 전체 그룹을 조회합니다.")
    @GetMapping("/")
    public ResponseDto<List<GroupDto.AllGroupList>> getAllGroup(@AuthenticationPrincipal UserDetailsImpl userDetails) {
        return groupService.getAllGroup(userDetails);
    }

    @Operation(summary = "태그별 전체 그룹 조회", description = "생성된 전체 그룹을 태그별로 조회합니다.")
    @GetMapping("/tag")
    public ResponseDto<List<GroupDto.AllGroupList>> getAllGroupByTag(@AuthenticationPrincipal UserDetailsImpl userDetails, @RequestParam(value = "tag") String keyword) {
        return groupService.getAllGroupByTag(userDetails, keyword);
    }

    @Operation(summary = "그룹 상세 조회", description = "groupId에 해당하는 그룹을 조회합니다.")
    @GetMapping("/{groupId}")
    public ResponseDto<GroupDto.Response> getGroupDetail(@PathVariable Long groupId, @AuthenticationPrincipal UserDetailsImpl userDetails) {
    ...
    public ResponseDto<GroupDto.Response> updateGroup(@PathVariable Long groupId,
    ...
    public ResponseDto<String> deleteGroup(@PathVariable Long groupId, @AuthenticationPrincipal UserDetailsImpl userDetails) {
